...
: QMainWindow(parent), m_scanGraph(NULL),
  ...
  m_cameraStored(false), m_filename(""),m_nb_step(0) {
...
void ViewerGui::addOctree(octomap::AbstractOcTree* tree, int id, octomap::pose6d origin,std::deque<octomap::point3d> ppath,std::deque<std::pair<octomap::point3d,double> > fpath,octomap::point3d start,octomap::point3d end,std::deque<std::pair<octomap::point3d,double> > obstacles) {
  // is id in use?
	  OcTreeRecord* r;
	  ...
	  if (foundRecord && r->octree->getTreeType().compare(tree->getTreeType()) !=0){
		...
		if (dynamic_cast<OcTree*>(tree)) {
		  ...
		  r->octree_drawer = new ColorOcTreeDrawer(ppath,fpath,start,end,obstacles);
		} else{
		...
		r->octree = tree;
		r->origin = origin;
		if (dynamic_cast<ColorOcTree*>(tree)) {
			((ColorOcTreeDrawer*)r->octree_drawer)->set(ppath,fpath,start,end,obstacles);
			((ColorOcTreeDrawer*)r->octree_drawer)->setOcTree(*tree,origin,id);
		}
	  } else {
		// add new record
		OcTreeRecord otr;
		...
		if (dynamic_cast<OcTree*>(tree)) {
		  ...
		  otr.octree_drawer = new ColorOcTreeDrawer(ppath,fpath,start,end,obstacles);
		} else{
		  OCTOMAP_ERROR("Could not create drawer for tree type %s\n", tree->getTreeType().c_str());
		}
		otr.octree = tree;
...
void ViewerGui::addOctree(octomap::AbstractOcTree* tree, int id,std::deque<octomap::point3d> ppath,std::deque<std::pair<octomap::point3d,double> > fpath,octomap::point3d start,octomap::point3d end,std::deque<std::pair<octomap::point3d,double> > obstacles) {
  octomap::pose6d o; // initialized to (0,0,0) , (0,0,0,1) by default
  addOctree(tree, id, o,ppath,fpath,start,end,obstacles);
}
...
void ViewerGui::openOcTree(){
	std::ifstream file(m_filename.c_str(), std::ios_base::in |std::ios_base::binary);
	 AbstractOcTree* tree;
	if (!file.is_open()){
	  std::cout<<"Filestream to "<< m_filename << " not open, nothing read."<<std::endl;
	  exit(1);
	} else {
...
//	for(octomap::ColorOcTree::tree_iterator it = ((octomap::ColorOcTree*)tree)->begin_tree(),	end=((octomap::ColorOcTree*)tree)->end_tree(); it!= end; ++it)
//	{
...
//		std::cout<< pt << "," << node_size << std::endl;
  ...
  if (tree){
    this->addOctree(tree, DEFAULT_OCTREE_ID,ppath,fpath,start,end,obstacles);
